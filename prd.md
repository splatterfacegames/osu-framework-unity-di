...
### 3.4 R3 Integration
- The package replaces `osu.Framework.Bindables` entirely with R3.
- `DependencyActivator` will natively support `ReactiveProperty<T>` and `ReadOnlyReactiveProperty<T>` for dependency injection.
...
2. **`Bindable<T>` (Reactive State):** We are entirely replacing osu!framework's `Bindable<T>` system with R3 (`ReactiveProperty<T>`).
